...
def main(params):
    ...
    if str.lower(params.random_restart) == 'true':
        save_dir = os.path.join('saved_models/' + params.attack + '_' + params.norm + '_' + str(params.eps) + '_steps_' + str(params.steps) + '_val_pgd_val_eps_' + str(params.val_eps) + '_val_steps_' + str(params.val_steps) + '_' + params.model + '_r_final')
    elif str.lower(params.random_restart) == 'false':
        save_dir = os.path.join('saved_models/' + params.attack + '_' + params.norm + '_' + str(params.eps) + '_steps_' + str(params.steps) + '_val_pgd_val_eps_' + str(params.val_eps) + '_val_steps_' + str(params.val_steps) + '_' + params.model + '_final')
    else:
        raise ValueError("Unknown '--random_restart' value.")
    os.makedirs(save_dir, exist_ok=True)
    step_size = (params.eps / params.steps) * 4 / 3
    val_step_size = (params.val_eps / params.val_steps) * 4 / 3
    if str.lower(params.attack) == 'pgd':
        ...
        if str.lower(params.norm) in ['linf', 'l_inf', 'l inf']:
            if str.lower(params.random_restart) == 'true':
                ...
                val_attack = LinfPGD(abs_stepsize=val_step_size, steps=params.val_steps, random_start=True)
            elif str.lower(params.random_restart) == 'false':
                attack = LinfFastGradientAttack(random_start=False)
                val_attack = LinfPGD(abs_stepsize=val_step_size, steps=params.val_steps, random_start=False)
            else:
                raise ValueError("Unknown '--random_restart' value.")
        elif str.lower(params.norm) in ['l2', 'l_2', 'l 2']:
            if str.lower(params.random_restart) == 'true':
                attack = L2FastGradientAttack(random_start=True)
                val_attack = L2PGD(abs_stepsize=val_step_size, steps=params.val_steps, random_start=True)
            elif str.lower(params.random_restart) == 'false':
                attack = L2FastGradientAttack(random_start=False)
                val_attack = L2PGD(abs_stepsize=val_step_size, steps=params.val_steps, random_start=False)
            else:
                raise ValueError("Unknown '--random_restart' value.")
        else:
            raise ValueError("Unknown '--norm' value.")
    ...
    for i in range(params.epoch):
        ...
        x_adv_val = batch_attack(imgs_val, labels_val, val_attack, fmodel, params.eps, 1000)
...
'''
    if str.lower(params.random_restart) == 'true':
        with open('saved_models/' + params.attack + '_' + params.norm + '_' + str(params.eps) + '_' + 'val_pgd' + '_' + 'val_eps_' + str(params.val_eps) + '_' + 'val_steps_' + str(params.val_steps) + '_' + params.model + '_r/runtime.txt',
                  "w") as f:
            f.write('Total runtime of last execution: ' + str(time.time() - runtime) + 's')
    else:
        with open('saved_models/' + params.attack + '_' + params.norm + '_' + str(params.eps) + '_' + 'val_pgd' + '_' + 'val_eps_' + str(params.val_eps) + '_' + 'val_steps_' + str(params.val_steps) + '_' + params.model + '/runtime.txt',
                  "w") as f:
            f.write('Total runtime of last execution: ' + str(time.time() - runtime) + 's\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--val_eps", type=float, default=0.3)
